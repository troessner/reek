...
describe ShouldReek, 'checking code in a string' do
  ...
  it 'reports the smells when should_not fails' do
    @matcher.matches?(@smelly_code).should be_true
    @matcher.failure_message_for_should_not.should include(ReportList.new([@smelly_code.sniff]).quiet_report)
  end
end
...
describe ShouldReek, 'checking code in a Dir' do
  ...
  it 'reports the smells when should_not fails' do
    @matcher.matches?(@smelly_dir).should be_true
    @matcher.failure_message_for_should_not.should include(ReportList.new(@smelly_dir.sniff.sniffers).quiet_report)
  end
end
...
describe ShouldReek, 'checking code in a File' do
  ...
  it 'reports the smells when should_not fails' do
    @matcher.matches?(@smelly_file).should be_true
    @matcher.failure_message_for_should_not.should include(ReportList.new([@smelly_file.sniff]).quiet_report)
  end
end
