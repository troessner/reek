...
describe UncommunicativeVariableName do
  ...
  context 'local variable name' do
    ...
    it 'reports one-letter variable name' do
      ...
      expect(src).to smell_of(UncommunicativeVariableName, 'name' => 'x')
    end

    it 'reports name of the form "x2"' do
      src = 'def simple(fred) x2 = jim(45) end'
      expect(src).to smell_of(UncommunicativeVariableName, 'name' => 'x2')
    end

    it 'reports long name ending in a number' do
      @bad_var = 'var123'
      src = "def simple(fred) #{@bad_var} = jim(45) end"
      expect(src).to smell_of(UncommunicativeVariableName, 'name' => @bad_var)
    end
    ...
    it 'reports a bad name inside a block' do
      ...
      expect(src).to smell_of(UncommunicativeVariableName, 'name' => 'q2')
    end
...
EOS
    ...
    it 'reports splatted block parameters correctly' do
      ...
      EOS
      expect(src).to smell_of(UncommunicativeVariableName, 'name' => 'y')
    end
